...
def ped_teste(chromossome, start, stop, list_samples):
    ...
    subprocess.call(PYTERA_PATH + "/static/tabix/tabix -f -p vcf -h " + baseName + " " + str(chromossome) + ":" + str(
...
def parse_fasta(chromossome, start, stop):
    basePath = getBasePath()
    for record in SeqIO.parse(PYTERA_PATH + '/static/downloads/1000GP/chr' + str(chromossome) + ".fasta", "fasta"):
        my_seq = record.seq
